...
def getpropertyraw(prop_id):
  ...
  try:
    ...
    if property_ in [0,1,2]:
      ROWS=dbSelect("select propertydata,registrationdata,flags from smartproperties sp where (protocol='Aryacoin' or protocol='Omni') and sp.propertyid=%s",[property_])

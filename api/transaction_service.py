...
def gettxjson(hash_id):
    ...
    try:
      ...
      if len(transaction_) == 64:
        #ROWS=dbSelect("select txj.txdata, extract(epoch from t.txrecvtime) from transactions t, txjson txj where t.txdbserialnum = txj.txdbserialnum and t.protocol != 'Aryacoin' and t.txhash=%s", [transaction_])
...
def getblocktxjson(block):
  ...
  try:
    ...
    try:
        ...
        ROWS=dbSelect("select txj.txdata from transactions t, txjson txj where t.txdbserialnum = txj.txdbserialnum and t.protocol != 'Aryacoin' and t.txblocknumber=%s", [block_])
    except Exception as e:
        return {'error':'This endpoint only consumes valid input. Invalid block'}
...
def getaddrhist(address,direction='both',page=1):
    ...
    query="select t.txhash from transactions t, addressesintxs atx where t.txdbserialnum = atx.txdbserialnum and t.protocol != 'Aryacoin' and atx.address='"+str(address_)+"'"
    role='address'
    if direction=='send':
      role='sender'
...
def gettransaction_OLD(hash_id):
    ...
    ROWS=dbSelect("select t.txhash,t.protocol,t.txdbserialnum,t.txtype,t.txversion,t.ecosystem,t.txrecvtime,t.txstate,t.txerrorcode,"
                  ...
                  "from transactions t, txjson txj where t.txdbserialnum = txj.txdbserialnum and t.protocol != 'Aryacoin' and t.txhash=%s", [transaction_])
    ...
    if (txType == 20 or txType == 22) and txValid:
      ...
      if txType == 20:
        ...
        if not cancel:
          ...
          ppc = Decimal( mpData['aryacoindesired'] ) / Decimal( mpData['amount'] )

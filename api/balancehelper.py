...
def getBalanceData(address,btcdata):
    ...
    try:
      ...
      ROWS=dbSelect("""select
                     ...
                     on f1.propertyid=sp.propertyid and (sp.protocol='Omni' or sp.protocol='Aryacoin')
                     order by f1.propertyid""",(addr,addr))
      balance_data = { 'balance': [] }
      for balrow in ROWS:
        cID = str(int(balrow[0])) #currency id
        sym_t = ('AYA' if cID == '0' else ('OMNI' if cID == '1' else ('T-OMNI' if cID == '2' else 'SP' + cID) ) ) #symbol template
